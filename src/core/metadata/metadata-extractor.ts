...
export function detectExtensionFromUrl(url: string): string | undefined {
  try {
    ...
    const extensionMatch = pathname.match(
      /\.(mp4|webm|mov|avi|mkv|flv|wmv|ogg|m4v|m2ts|ts|mpg|mpeg)(\?|$|#)/,
    );
    ...
    const extParam =
      urlObj.searchParams.get("ext") || urlObj.searchParams.get("format");
    if (
      extParam &&
      /^(mp4|webm|mov|avi|mkv|flv|wmv|ogg|m4v|m2ts|ts|mpg|mpeg)$/i.test(
        extParam,
      )
    ) {
 ...
 */
export function detectExtensionFromContentType(
  contentType: string,
): string | undefined {
...
export async function extractMetadataFromDirectBlob(
  ...
  logger.info(
    `Extracted metadata from direct blob: format=${metadata.format}, extension=${metadata.extension}`,
  );
