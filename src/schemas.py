...
class DownloaderParams(BaseModel):
    ...
    def validate_urls_provided(cls, v, values):
        ...
        if v is None and ('url' not in values or values['url'] is None) and ('urls' not in values or values['urls'] is None):
    ...
    def validate_filename(cls, v, values):
        if v and values.get('urls'):
            raise ValueError("Custom filename can only be used with single URL download")
        return v
